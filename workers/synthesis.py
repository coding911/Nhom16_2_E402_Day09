...
def synthesize(task: str, chunks: list, policy_result: dict, judge_feedback: str = "") -> dict:
    """
    Tổng hợp câu trả lời từ chunks và policy context.
    Nếu judge_feedback có giá trị → đây là retry, thêm feedback vào prompt.
    ...
    # Build user content — thêm judge feedback nếu đang retry
    if judge_feedback:
        user_content = f"""Câu hỏi: {task}

{context}

⚠️ Lần trả lời trước chưa đạt yêu cầu. Hãy cải thiện theo phản hồi sau:
{judge_feedback}

Hãy trả lời lại câu hỏi, khắc phục đúng điểm được nêu ở trên."""
    else:
        user_content = f"""Câu hỏi: {task}
...
def run(state: dict) -> dict:
    ...
    try:
        ...
        judge_iter = state.get("judge_iterations", 0)
        result = synthesize(task, chunks, policy_result, judge_feedback=judge_feedback)
        state["final_answer"] = result["answer"]
        state["sources"] = result["sources"]
        state["confidence"] = result["confidence"]
        # Reset feedback sau khi đã dùng (tránh truyền lại lần sau)
        ...
        worker_io["output"] = {
            ...
            "is_retry": judge_iter > 0,
        }
        state["history"].append(
            f"[{WORKER_NAME}] answer generated (retry={judge_iter}), "
