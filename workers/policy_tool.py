...
def run(state: dict) -> dict:
    ...
    try:
        ...
        # Step 1: Nếu chưa có chunks, gọi MCP search_kb để bổ sung context
        if not chunks and needs_tool:
            mcp_result = _call_mcp_tool("search_kb", {"query": task, "top_k": 3})
            state["mcp_tools_used"].append(mcp_result)
            state["history"].append(f"[{WORKER_NAME}] called MCP search_kb (no prior chunks)")

            if mcp_result.get("output") and mcp_result["output"].get("chunks"):
                chunks = mcp_result["output"]["chunks"]
                state["retrieved_chunks"] = chunks

        # Step 2: Gọi MCP check_access_permission khi task liên quan đến cấp quyền
        access_keywords = ["cấp quyền", "access level", "level 3", "level 2",
                           "admin access", "elevated access", "contractor", "quyền truy cập"]
        if needs_tool and any(kw in task_lower for kw in access_keywords):
            # Xác định access level từ task
            ...
            is_emergency = any(kw in task_lower for kw in ["khẩn cấp", "emergency", "urgent", "p1"])
            ...
            state["history"].append(
                ...
                f"(level={access_level}, role={requester_role}, emergency={is_emergency})"
            )

            # Gắn kết quả vào policy_result để synthesis dùng
            state["mcp_access_result"] = mcp_result.get("output", {})

        # Step 3: Gọi MCP get_ticket_info khi task đề cập ticket cụ thể
        if needs_tool and any(kw in task_lower for kw in ["ticket", "p1", "jira", "it-"]):
            mcp_result = _call_mcp_tool("get_ticket_info", {"ticket_id": "P1-LATEST"})
            state["mcp_tools_used"].append(mcp_result)
            state["history"].append(f"[{WORKER_NAME}] called MCP get_ticket_info")

        # Step 4: Phân tích policy (rule-based)
        policy_result = analyze_policy(task, chunks)

        # Bổ sung access info vào policy_result nếu có
        if state.get("mcp_access_result"):
            access_info = state["mcp_access_result"]
            policy_result["access_check"] = {
                "can_grant": access_info.get("can_grant"),
                "required_approvers": access_info.get("required_approvers", []),
                "emergency_override": access_info.get("emergency_override", False),
                "notes": access_info.get("notes", []),
            }

        state["policy_result"] = policy_result

        mcp_call_count = len(state["mcp_tools_used"])
        worker_io["output"] = {
            "policy_applies": policy_result["policy_applies"],
            "exceptions_count": len(policy_result.get("exceptions_found", [])),
            "mcp_calls": mcp_call_count,
            "mcp_tools_called": [m["tool"] for m in state["mcp_tools_used"]],
        }
        state["history"].append(
            f"[{WORKER_NAME}] policy_applies={policy_result['policy_applies']}, "
            f"exceptions={len(policy_result.get('exceptions_found', []))}, "
            f"mcp_calls={mcp_call_count}"
        )
